...
(function (window, document) {
    ...
    // =========================================
    // 5. WAKE LOCK (Layar Tetap Menyala)
    // =========================================
    async function requestWakeLock() {
        if ('wakeLock' in navigator && appSettings.keepScreenOn && $('quran-view').classList.contains('active')) {
            try {
    ...
    function loadBookmarks() {
        ...
        if (savedLR) {
            if (Array.isArray(savedLR?.items)) {
                ...
                appLastRead = { items: savedLR, timestamp: Date.now() };
            } else {
                appLastRead = { items: [savedLR], timestamp: savedLR.timestamp || Date.now() };
            }
        } else {
    ...
    // =========================================
    // 10. BOOKMARK & FOLDER MANAGEMENTS
    // =========================================
    ...
    // =========================================
    // 13. QURAN READING, PAGE CONTENT, SETTINGS
    // =========================================
            ...
            if (remoteData.lastRead) {
                ...
                if (!rLastRead.items) {
                    if (Array.isArray(rLastRead)) {
                        rLastRead = { items: rLastRead, timestamp: rLastRead[0]?.timestamp || Date.now() };
                    } else {
                        rLastRead = { items: [rLastRead], timestamp: rLastRead.timestamp || Date.now() };
                    }
                }
    ...
    // =========================================
    // 16. PWA GESTURE & GLOBAL EVENT DELEGATION
    // =========================================
